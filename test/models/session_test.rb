...
class WorkSessionTest < ActiveSupport::TestCase
  ...
  test "live_seconds adds elapsed time when running" do
    ...
    travel_to(base_time) do
      work_session = WorkSession.new(total_seconds: 100, started_at: Time.current, ended_at: nil)
      travel 2.seconds
  ...
  test "hour_price returns 0 when target_hours is 0" do
    work_session = WorkSession.new(target_price: 1000, target_hours: 0)
  ...
  test "hour_price returns correct value when target_price and target_hours are positive" do
    work_session = WorkSession.new(target_price: 1000, target_hours: 2)
    assert_equal 500, work_session.hour_price
  end

  test "hour_price returns correct decimal when target_hours is 1.5" do
    work_session = WorkSession.new(target_price: 1000, target_hours: 1.5)
  ...
  test "now_price multiplies hour_price and live_hours when stopped" do# 停止中の整数ケース
    work_session = WorkSession.new(target_price: 1000, target_hours: 2, total_seconds: 7200)
    assert_equal 1000, work_session.now_price
  end

  test "now_price returns correct value when live_hours is decimal" do  # 停止中の少数ケース
    work_session = WorkSession.new(target_price: 1200, target_hours: 2, total_seconds: 5400)
  ...
  test "now_price during run adds elapsed hours (integer case)" do# 実行中の整数ケース
    base_time = Time.current.change(usec: 0) # ミリ秒の誤差を消す
    travel_to(base_time) do
      work_session = WorkSession.new(started_at: Time.current, ended_at: nil, target_price: 1000, target_hours: 2, total_seconds: 3600)
      travel 7200.seconds
  ...
  test "now_price calculates correctly when running with fractional hour" do# 実行中の少数ケース
    base_time = Time.current.change(usec: 0) # ミリ秒の誤差を消す
    travel_to(base_time) do
      work_session = WorkSession.new(started_at: Time.current, ended_at: nil, target_price: 1200, target_hours: 2, total_seconds: 1800)
      travel 3600.seconds
      assert_in_delta 900, work_session.now_price, 0.01
    end
  end
  ...
  test "progress_ratio is 0.0 when target_price is zero" do
    work_session = WorkSession.new(target_price: 0, target_hours: 2, total_seconds: 7200)
  ...
  test "progress_ratio returns fractional value below 1.0" do
    ...
    work_session = WorkSession.new(target_price: 1000, target_hours: 2, total_seconds: 2160)
  ...
  test "progress_ratio does not exceed 1.0 even when now_price > target_price" do
    ...
    work_session = WorkSession.new(target_price: 1000, target_hours: 2, total_seconds: 10800)
    assert_equal 1.0, work_session.progress_ratio
  end
